# nf-core/controldna: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/controldna/usage](https://nf-co.re/controldna/usage)
...
```console
nextflow run nf-core/controldna --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/controldna
```
...
First, go to the [nf-core/controldna releases page](https://github.com/nf-core/controldna/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
